...
function CheckPDF($FILES)
{
    try {
        if ($FILES['ReportUploadPDF']['error'] == 0) {
            ...
            move_uploaded_file($pdf_tmp_name, "./uploads/" . $pdf_name);
            exec("chown libobioadmin ./uploads/" . $pdf_name); // Change file owner to libobioadmin
            exec("chmod 777 ./uploads/" . $pdf_name); // Change file mode to 777
            return $pdf_name;
        }
        return false;
    } catch (Exception $th) {
