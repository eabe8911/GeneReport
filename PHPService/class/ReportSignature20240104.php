...
class ReportSignature
{
        ...
        switch ($UserInfo['permission']) {
            ...
            case '2': // Medical Examiner
                if ($TemplateInfo['Approve'] == 1 || $TemplateInfo['Approve'] == 2) {
                    ...
                    throw new Exception("Permission denied", 2);
                }
                break;
            case '3': // Doctor
                if ($TemplateInfo['Approve'] == 2) {
                    ...
                    throw new Exception("此份報告不須醫師簽核", 3);
                }
                break;
            case '4': // Admin
                throw new Exception("Permission denied", 4);
            default:
                throw new Exception("Permission denied", 5);
        }
    }
    ...
    function CallWebService($pdfPath, $signaturePath, $position, $PageNumber, $date, $UpdateFileName)
    {
        try {
            ...
            $token = 'libobio_token';
            $client = new Client(['base_uri' => 'http://flask-api:5000']);
            $data = [
                'pdf_file' => $pdfBase64,
