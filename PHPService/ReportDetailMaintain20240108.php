    ...
    try {
        ...
        switch ($ReportMode) {
            case 'ADD':
                ...
                $log->SaveLog("ADD", $Username, "UpdateReportInfo", date("Y-m-d H:i:s"), $logDataJson);
                $messageData = [
            ...
            case 'EDIT':
                if ($report->UpdateReport($_POST)) {
                    ...
                    CheckPDF($_FILES);
                    $log->SaveLog("UPDATE", $Username, "UpdateReportInfo", date("Y-m-d H:i:s"), $logDataJson);
                }
                $messageData = [
                    'report_id' => $_POST['ReportID'],
                    'status' => 'update'
            ...
            case 'DELETE':
                if ($report->DeleteReport($_POST)) {
                    ...
                    $log->SaveLog("DELETE", $Username, "UpdateReportInfo", date("Y-m-d H:i:s"), $logDataJson);

                    // $log->SaveLog("DELETE", $Username, "UpdateReportInfo", date("Y-m-d H:i:s"), json_encode($_POST));
                }
                $messageData = [
                    'report_id' => $_POST['ReportID'],
                    'status' => 'delete'
...
if ($PDFFile == '') {
    ...
    // $Email->SendEmail($ReportID, $CustomerEmail, $CustomerName);
}
...
echo '<script>
    ...
    document.getElementById("email_report").addEventListener("submit", function(event){
        const confirmMessage = confirm(`確定要寄出報告嗎?\n請再確認資訊是否正確：\n報告編號：' . $ReportID . '\n客戶姓名：' . $CustomerName . '\n客戶信箱：' . $CustomerEmail . '`);
        if(!confirmMessage){
